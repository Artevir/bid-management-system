...
async function generateWord(exportData: any, filename: string): Promise<Uint8Array> {
  ...
  // 使用项目名称作为标题
  const projectName =
    exportData.basicInfo?.projectName || exportData.framework?.[0]?.title || '标书解读结果';
  children.push(
    new Paragraph({
      text: projectName,
  ...
  if (exportData.basicInfo) {
    ...
    const infoRows = Object.entries(exportData.basicInfo)
  ...
  if (exportData.feeInfo) {
    ...
    const feeRows = Object.entries(exportData.feeInfo)
      .filter(([k, v]) => v !== null && v !== undefined && v !== '')
      .map(
        ([k, v]) =>
          new TableRow({
    ...
    const subRows = [
      new TableRow({
        children: [
          new TableCell({
            children: [
              new Paragraph({ children: [new TextRun({ text: '要求类型', bold: true })] }),
            ],
          }),
          new TableCell({
            children: [
              new Paragraph({ children: [new TextRun({ text: '具体要求', bold: true })] }),
            ],
          }),
          new TableCell({
            children: [new Paragraph({ children: [new TextRun({ text: '份数', bold: true })] })],
          }),
        ],
      }),
      ...exportData.submissionRequirements.map(
        (item: any) =>
          new TableRow({
            children: [
  ...
  if (exportData.framework?.length) {
    ...
    const frameRows = [
      new TableRow({
        children: [
          new TableCell({
            children: [new Paragraph({ children: [new TextRun({ text: '章节', bold: true })] })],
          }),
          new TableCell({
            children: [new Paragraph({ children: [new TextRun({ text: '标题', bold: true })] })],
          }),
          new TableCell({
            children: [new Paragraph({ children: [new TextRun({ text: '页码', bold: true })] })],
          }),
        ],
      }),
      ...exportData.framework.map(
        (item: any) =>
          new TableRow({
            children: [
    ...
    const timeRows = [
      new TableRow({
        children: [
          new TableCell({
            children: [
              new Paragraph({ children: [new TextRun({ text: '节点名称', bold: true })] }),
            ],
          }),
          new TableCell({
          ...
          new TableCell({
            children: [new Paragraph({ children: [new TextRun({ text: '地点', bold: true })] })],
          }),
        ],
      }),
      ...exportData.timeNodes.map(
        (item: any) =>
          new TableRow({
            children: [
  ...
  if (exportData.technicalSpecs?.length) {
    ...
    const specRows = [
      new TableRow({
        children: [
          new TableCell({
            children: [new Paragraph({ children: [new TextRun({ text: '类别', bold: true })] })],
          }),
          new TableCell({
            children: [new Paragraph({ children: [new TextRun({ text: '名称', bold: true })] })],
          }),
          new TableCell({
            children: [new Paragraph({ children: [new TextRun({ text: '要求', bold: true })] })],
          }),
        ],
      }),
      ...exportData.technicalSpecs.map(
        (item: any) =>
          new TableRow({
            children: [
    ...
    const scoreRows = [
      new TableRow({
        children: [
          ...
          new TableCell({
            children: [new Paragraph({ children: [new TextRun({ text: '分值', bold: true })] })],
          }),
          new TableCell({
            children: [
              new Paragraph({ children: [new TextRun({ text: '评分标准', bold: true })] }),
            ],
          }),
        ],
      }),
      ...exportData.scoringItems.map(
        (item: any) =>
          new TableRow({
            children: [
              new TableCell({ children: [new Paragraph({ text: item.category || '' })] }),
    ...
    const checkRows = [
      new TableRow({
        children: [
          ...
          new TableCell({
            children: [
              new Paragraph({ children: [new TextRun({ text: '是否具备', bold: true })] }),
            ],
          }),
        ],
      }),
...
async function generatePdf(exportData: any): Promise<Uint8Array> {
  pdfmake.setFonts({
    Helvetica: {
      normal: 'Helvetica',
      bold: 'Helvetica-Bold',
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const [interpretation, technicalSpecs, scoringItems, checklist, framework] = await Promise.all([
      ...
      modules.includes('specs') || modules.includes('all')
        ? getTechnicalSpecs(interpretationId)
        : [],
      modules.includes('scoring') || modules.includes('all')
        ? getScoringItems(interpretationId)
        : [],
      modules.includes('checklist') || modules.includes('all')
        ? getChecklist(interpretationId)
        : [],
      modules.includes('framework') || modules.includes('all')
    ...
    if (format === 'pdf') {
      const bytes = await generatePdf(exportData);
