...
export async function POST(req: NextRequest) {
  return withAuth(req, async (request, userId) => {
    try {
      ...
      const transferToNum =
        transferTo !== undefined && transferTo !== null ? Number(transferTo) : null;
      ...
      if (
        isTransferAction &&
        (!Number.isInteger(transferToNum) || (transferToNum as number) <= 0)
      ) {
      ...
      await db.transaction(async (tx) => {
        ...
        const newStatus =
          action === 'approve' ? 'completed' : action === 'reject' ? 'rejected' : 'pending';
      ...
      if (error instanceof Error) {
        if (
          error.message === 'TASK_NOT_FOUND_OR_PROCESSED' ||
          error.message === 'TASK_CONCURRENTLY_UPDATED'
        ) {
